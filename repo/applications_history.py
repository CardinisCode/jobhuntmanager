class ApplicationsHistoryRepository:
    ...
    def grabTop10ApplicationsFromHistory(self, user_id):
        cursor = self.db.cursor()
        result = cursor.execute("SELECT id, date, job_ref, company_name, job_role, platform, employment_type, interview_stage, contact_received, salary FROM application_history WHERE user_id = ? ORDER BY date DESC LIMIT 10", (user_id,))
        self.db.commit()
