...
def cleanup_job_offer(job_offers_details, count):
    ...
    for heading, value in job_offers_details[count].items():
        if heading == "offer_response":
            job_offers_details[count][heading] = cleanup_offer_response(value, company_name)
            
            # If the user has accepted the job offer, we can upate the dictionary's key: 'offer_accepted'
            if value == "Offer Accepted!":
                job_offers_details[count]["offer_accepted"] = True
        elif heading == "starting_date":
            job_offers_details[count][heading] = cleanup_date_format(value)
        elif heading == "company_name" or heading == "job_role":
            job_offers_details[count][heading] = cleanup_field_value(value)
        else:
            continue
